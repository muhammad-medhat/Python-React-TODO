...
function createElement(elType, elID, elVal, type='text'){
...
function createEdit(id, tName, tCont, tProg){
    ...
    // return [tNameInp, tContInp, tProgInp]
        ...
        const tProgInp = createElement('input', `editP-${id}`, tProg, 'number')
        tProgInp.min=0
        tProgInp.max=100
        const progDiv = elmnt('div', 'task-prog', tProgInp)
        console.log('==========')
        console.log(progDiv)
...
function afterEdit(id, jsn){
    ...
    progDiv.innerHTML = `Overall Progress: ${jsn['body']['prog']} %`

    progSpan = elmnt('span', 'prog-bar', '')
    progSpan.dataset.id=jsn['body']['prog']
    progDiv.style.width = jsn['body']['prog']
    console.log(progSpan);
